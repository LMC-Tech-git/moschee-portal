...
export const tvSettingsSchema = z
  .object({
    ...
    tv_module_counts: z.object({
      ...
      posts: z.number().int().min(1).max(10).optional(),
      campaigns: z.number().int().min(1).max(10).optional(),
      qr_donate: z.number().int().min(1).max(10).optional(),
      announcement: z.number().int().min(1).max(10).optional(),
    }).partial(),
